...
import { LineChart, Gauge } from '../Charts';
    ...
    <div className={`bg-card border border-border rounded-xl p-6 space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className={`${healthBgColor} border border-current/20 rounded-lg p-3`}>
          ...
          <div className={`text-2xl font-bold ${healthColor}`}>{CPI.toFixed(2)}</div>
        ...
        <div className={`${healthBgColor} border border-current/20 rounded-lg p-3`}>
          ...
          <div className={`text-2xl font-bold ${healthColor}`}>{SPI.toFixed(2)}</div>
          <div className="text-xs text-muted-foreground mt-1">
        ...
        <div className={`${CV >= 0 ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-red-50 dark:bg-red-900/20'} border border-current/20 rounded-lg p-3`}>
          ...
          <div className={`text-2xl font-bold ${CV >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            {fmtQ(CV)}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
        ...
        <div className={`${SV >= 0 ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-red-50 dark:bg-red-900/20'} border border-current/20 rounded-lg p-3`}>
          ...
          <div className={`text-2xl font-bold ${SV >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            {fmtQ(SV)}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
      ...
      <div className="border-t border-border/50 pt-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2">
            <div className="bg-muted/50 rounded-lg p-3 h-40 flex items-center justify-center">
      ...
      <div className={`${healthBgColor} border border-current/20 rounded-lg p-4`}>
        ...
        <ul className="text-xs text-foreground/80 space-y-1">
          <li>
            • <span className="font-medium">CPI {CPI.toFixed(2)}:</span> Por cada quetzal gastado, se
            ganó {fmtQ(EV / AC)} de valor
          </li>
          <li>
            • <span className="font-medium">SPI {SPI.toFixed(2)}:</span> Proyecto
