...
const CuentasPagar: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (!formProyectoId) errors.proyectoId = t('cuentas_pagar.proyecto_requerido', 'Proyecto requerido');
    if (!formProveedor.trim()) errors.proveedor = t('cuentas_pagar.proveedor_requerido', 'Proveedor requerido');
    if (!formConcepto.trim()) errors.concepto = t('cuentas_pagar.concepto_requerido', 'Concepto requerido');
    ...
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <h1 className="text-lg sm:text-xl font-black text-foreground flex items-center gap-2">
          <DollarSign className="w-5 h-5 sm:w-6 sm:h-6 text-rose-500 dark:text-rose-400" aria-hidden="true" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-emerald-50 dark:bg-emerald-950/40 rounded-xl border border-emerald-100 dark:border-emerald-900/50 p-4">
          <div className="text-xs text-emerald-600 dark:text-emerald-400">{t('cuentas_pagar.pagadas')}</div>
          <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">{stats.pagadas}</div>
          <div className="text-xs text-emerald-600 dark:text-emerald-400">{fmtQ(stats.montoPagado)}</div>
        </div>
        <div className="bg-amber-50 dark:bg-amber-950/40 rounded-xl border border-amber-100 dark:border-amber-900/50 p-4">
          <div className="text-xs text-amber-600 dark:text-amber-400">{t('cuentas_pagar.pendientes')}</div>
          <div className="text-2xl font-bold text-amber-700 dark:text-amber-300">{stats.pendientes}</div>
          <div className="text-xs text-amber-600 dark:text-amber-400">{stats.total > 0 ? Math.round((stats.pendientes / stats.total) * 100) : 0}% {t('cuentas_pagar.del_total')}</div>
        </div>
        <div className="bg-rose-50 dark:bg-rose-950/40 rounded-xl border border-rose-100 dark:border-rose-900/50 p-4">
          <div className="text-xs text-rose-600 dark:text-rose-400">{t('cuentas_pagar.vencidas')}</div>
          <div className="text-2xl font-bold text-rose-700 dark:text-rose-300">{stats.vencidas}</div>
          <div className="text-xs text-rose-600 dark:text-rose-400">{stats.vencidas > 0 ? t('cuentas_pagar.requiere_pago_inmediato') : t('cuentas_pagar.sin_vencidas')}</div>
        </div>
      </div>
              ...
              <tbody>
                {filtered.map(c => (
                  <tr key={c.id} className="border-b border-border hover:bg-muted/50">
                     <td className="p-2 font-medium truncate" title={c.proveedorNombre}>{c.proveedorNombre}</td>
                     <td className="p-2 text-muted-foreground truncate" title={c.concepto}>{c.concepto}</td>
                    <td className="p-2 text-right font-medium">{fmtQ(c.monto)}</td>
                    <td className="p-2 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        c.estado === 'pagada' ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300' : c.estado === 'vencida' ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300'
                    ...
                    <td className="p-2 text-right">
                      {c.estado !== 'pagada' && (
                        <button onClick={() => handleMarkAsPaid(c.id)} className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={t('cuentas_pagar.marcar_pagada')}>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in duration-200" role="dialog" aria-modal="true" aria-label={editingId ? t('cuentas_pagar.editar') : t('cuentas_pagar.nueva_cuenta')}>
          <div className="bg-card rounded-lg p-6 w-full max-w-md shadow-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid gap-3">
              <div>
                <label htmlFor="pagar-proyecto" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.col_proveedor')}</label>
                <select id="pagar-proyecto" value={formProyectoId} onChange={e => setFormProyectoId(e.target.value)} className={INPUT}>
              ...
              <div>
                <label htmlFor="pagar-proveedor" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.col_proveedor')}</label>
                <input id="pagar-proveedor" value={formProveedor} onChange={e => setFormProveedor(e.target.value)} className={INPUT} placeholder={t('cuentas_pagar.proveedor_placeholder')} />
                {formErrors.proveedor && <p className="text-xs text-red-500 mt-0.5">{formErrors.proveedor}</p>}
              </div>
              <div>
                <label htmlFor="pagar-concepto" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.col_concepto')}</label>
                <input id="pagar-concepto" value={formConcepto} onChange={e => setFormConcepto(e.target.value)} className={INPUT} placeholder={t('cuentas_pagar.concepto_placeholder')} />
                {formErrors.concepto && <p className="text-xs text-red-500 mt-0.5">{formErrors.concepto}</p>}
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label htmlFor="pagar-monto" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.col_monto')}</label>
                  <input id="pagar-monto" type="number" inputMode="decimal" value={formMonto || ''} onChange={e => setFormMonto(+e.target.value)} className={INPUT} />
                  {formErrors.monto && <p className="text-xs text-red-500 mt-0.5">{formErrors.monto}</p>}
                </div>
                <div>
                  <label htmlFor="pagar-vencimiento" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.col_vencimiento')}</label>
                  <input id="pagar-vencimiento" type="date" value={formFechaVencimiento} onChange={e => setFormFechaVencimiento(e.target.value)} className={INPUT} />
                  {formErrors.fechaVencimiento && <p className="text-xs text-red-500 mt-0.5">{formErrors.fechaVencimiento}</p>}
                </div>
              </div>
              <div>
                <label htmlFor="pagar-estado" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.col_estado')}</label>
                <select id="pagar-estado" value={formEstado} onChange={e => setFormEstado(e.target.value)} className={INPUT}>
                  <option value="pendiente">{t('cuentas_pagar.pendiente')}</option>
                  <option value="pagada">{t('cuentas_pagar.pagada')}</option>
                  <option value="vencida">{t('cuentas_pagar.vencida')}</option>
                </select>
              ...
              <div>
                <label htmlFor="pagar-factura-url" className="text-xs text-muted-foreground mb-1 block">{t('cuentas_pagar.factura_url', 'Factura URL (opcional)')}</label>
                <input id="pagar-factura-url" value={formFacturaUrl} onChange={e => setFormFacturaUrl(e.target.value)} className={INPUT} placeholder="https://..." />
              </div>
              <div className="flex gap-2">
