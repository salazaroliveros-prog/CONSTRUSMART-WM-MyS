        ...
        {tab === 'insumos' && (
          <div>
            ...
            {filteredInsumos.length === 0 ? (
              <Empty description={t('apu.sin_insumos')} />
            ) : (
            <><div className="text-xs text-muted-foreground mb-2">{filteredInsumos.length} insumos · Precios de referencia INSIVUMEH / MOP</div>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Insumo</th>
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Categoría</th>
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Unidad</th>
                    <th className="text-right py-2 px-2 font-medium text-muted-foreground">Precio Ref.</th>
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Rubro</th>
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Últ. Actualización</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {tab === 'rendimientos' && (
          <div>
            ...
            {filteredRendimientos.length === 0 ? (
              <Empty description={t('apu.sin_rendimientos')} />
            ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Actividad</th>
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Cuadrilla</th>
                    <th className="text-right py-2 px-2 font-medium text-muted-foreground">Rendimiento</th>
                    <th className="text-left py-2 px-2 font-medium text-muted-foreground">Unidad</th>
                  </tr>
                </thead>
                <tbody>
...
<option value="">- Sin proyecto (referencia general) -</option>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
              {([{ key: 'indirectos' as const, label: 'Indirectos', desc: 'Gastos generales de obra' }, { key: 'administracion' as const, label: 'Administración', desc: 'Gastos administrativos' }, { key: 'imprevistos' as const, label: 'Imprevistos', desc: 'Contingencias' }, { key: 'utilidad' as const, label: 'Utilidad', desc: 'Margen de ganancia' }]).map(item => (
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-xl p-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div><span className="text-xs text-orange-600 font-medium">Factor multiplicador:</span><span className="text-xl font-bold text-orange-700 ml-2">{((factor.indirectos + factor.administracion + factor.imprevistos + factor.utilidad) / 100 + 1).toFixed(2)}</span></div>
        ...
        {tab === 'dosificacion' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              ...
              <div><label className="text-xs text-muted-foreground mb-1 block">Aditivos</label><select value={dosificacion.aditivos} onChange={e => setDosificacion(d => ({ ...d, aditivos: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="ninguno">Ninguno</option><option value="acelerador">Acelerador</option><option value="retardador">Retardador</option><option value="plastificante">Plastificante</option><option value="impermeabilizante">Impermeabilizante</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo de Curado</label><select value={dosificacion.curado} onChange={e => setDosificacion(d => ({ ...d, curado: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="normal">Normal</option><option value="acelerado">Acelerado</option><option value="prolongado">Prolongado</option></select></div>
            ...
            {resultadoDosificacion && (
              <div className="mt-4 space-y-3">
                ...
                <div className="bg-gradient-to-r from-primary to-warning rounded-xl p-4 shadow-sm">
                  <div className="flex items-center justify-between"><div><span className="text-xs text-white/80">Costo Total</span><div className="text-2xl font-bold text-white">Q{resultadoDosificacion.costoTotal.toFixed(2)}</div></div><div className="text-right"><span className="text-xs text-white/80">Factor Ajuste</span><div className="text-lg font-bold text-white">{resultadoDosificacion.factorAjuste.toFixed(2)}</div></div></div>
                </div>
          ...
          <div>
            ...
            <div className="bg-gradient-to-r from-primary to-warning rounded-xl p-4 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right"><span className="text-xs text-white/80">Factor</span><div className="text-lg font-bold text-white">{calculos.factorMultiplicador.toFixed(2)}</div></div>
              </div>
            </div>
        ...
        {tab === 'historico' && (
          <div>
            ...
            {historial.length > 0 ? (<>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-border text-muted-foreground"><th className="text-left py-2 px-2 font-medium text-muted-foreground">Fecha</th><th className="text-right py-2 px-2 font-medium text-muted-foreground">Cemento</th><th className="text-right py-2 px-2 font-medium text-muted-foreground">Hierro 3/8"</th><th className="text-right py-2 px-2 font-medium text-muted-foreground">Arena</th><th className="text-right py-2 px-2 font-medium text-muted-foreground">Block</th></tr></thead>
              ...
              <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
                <div className="bg-muted/30 rounded-xl p-3 border border-border"><div className="text-xs text-muted-foreground">Cemento</div><div className="text-sm font-bold text-orange-600">Q{historial[historial.length - 1].cemento.toFixed(0)}<span className="text-xs ml-1 text-red-500">↑ {(((historial[historial.length - 1].cemento - historial[0].cemento) / (historial[0].cemento || 1)) * 100).toFixed(1)}%</span></div></div>
                <div className="bg-muted/30 rounded-xl p-3 border border-border"><div className="text-xs text-muted-foreground">Hierro 3/8"</div><div className="text-sm font-bold text-blue-600">Q{historial[historial.length - 1].hierro.toFixed(0)}<span className="text-xs ml-1 text-red-500">↑ {(((historial[historial.length - 1].hierro - historial[0].hierro) / (historial[0].hierro || 1)) * 100).toFixed(1)}%</span></div></div>
                <div className="bg-muted/30 rounded-xl p-3 border border-border"><div className="text-xs text-muted-foreground">Arena</div><div className="text-sm font-bold text-emerald-600">Q{historial[historial.length - 1].arena.toFixed(0)}<span className="text-xs ml-1 text-red-500">↑ {(((historial[historial.length - 1].arena - historial[0].arena) / (historial[0].arena || 1)) * 100).toFixed(1)}%</span></div></div>
                <div className="bg-muted/30 rounded-xl p-3 border border-border"><div className="text-xs text-muted-foreground">Block</div><div className="text-sm font-bold text-blue-600">Q{historial[historial.length - 1].block.toFixed(0)}<span className="text-xs ml-1 text-red-500">↑ {(((historial[historial.length - 1].block - historial[0].block) / (historial[0].block || 1)) * 100).toFixed(1)}%</span></div></div>
              </div>
        ...
        {tab === 'movimientoTierra' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo</label><select value={movimientoTierra.tipo} onChange={e => setMovimientoTierra(d => ({ ...d, tipo: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="excavacion">Excavación</option><option value="relleno">Relleno</option><option value="compactacion">Compactación</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Suelo</label><select value={movimientoTierra.suelo} onChange={e => setMovimientoTierra(d => ({ ...d, suelo: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="relleno">Relleno</option><option value="arcilla">Arcilla</option><option value="arena">Arena</option><option value="roca_blanda">Roca Blanda</option><option value="roca_dura">Roca Dura</option></select></div>
              ...
              <div><label className="text-xs text-muted-foreground mb-1 block">Acceso</label><select value={movimientoTierra.acceso} onChange={e => setMovimientoTierra(d => ({ ...d, acceso: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="retroexcavadora">Retroexcavadora</option><option value="cargador">Cargador Frontal</option><option value="manual">Manual</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Drenaje</label><select value={movimientoTierra.drenaje} onChange={e => setMovimientoTierra(d => ({ ...d, drenaje: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="seco">Seco</option><option value="agua">Con Agua</option><option value="lodos">Con Lodos</option></select></div>
        ...
        {tab === 'parametrosClimaticos' && (
          <div>
            ...
            {resultadoClimaticos && (
              <div className="mt-4 space-y-3">
                <div className="bg-muted/30 rounded-xl p-4 border border-border"><h3 className="font-bold text-muted-foreground text-xs mb-3">Factores Climáticos</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><div className="bg-blue-50 rounded-lg p-3 border border-blue-100"><div className="text-xs text-blue-600 mb-1">Factor Curado</div><div className="text-lg font-bold text-blue-700">{resultadoClimaticos.factorCurado?.toFixed(2) || '1.00'}</div></div><div className="bg-amber-50 rounded-lg p-3 border border-amber-100"><div className="text-xs text-amber-600 mb-1">Factor Rendimiento</div><div className="text-lg font-bold text-amber-700">{resultadoClimaticos.factorRendimiento?.toFixed(2) || '1.00'}</div></div><div className="bg-stone-50 rounded-lg p-3 border border-stone-100"><div className="text-xs text-stone-600 mb-1">Factor Protección</div><div className="text-lg font-bold text-stone-700">{resultadoClimaticos.factorProteccion?.toFixed(2) || '1.00'}</div></div></div></div>
                <div className="bg-gradient-to-r from-primary-to-warning rounded-xl p-4 shadow-sm"><div className="flex items-center justify-between"><div><span className="text-xs text-white/80">Ajuste Estacional</span><div className="text-2xl font-bold text-white">{resultadoClimaticos.factorAjusteEstacional?.toFixed(2) || '1.00'}</div></div><div className="text-right"><span className="text-xs text-white/80">Observaciones</span><div className="text-sm font-bold text-white">{resultadoClimaticos.observaciones || ''}</div></div></div></div>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'pavimentos' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              <div><label className="text-xs text-muted-foreground mb-1 block">Uso</label><select value={pavimento.uso} onChange={e => setPavimento(d => ({ ...d, uso: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="peatonal">Peatonal</option><option value="vehicular_liviano">Vehicular Liviano</option><option value="vehicular_medio">Vehicular Medio</option><option value="vehicular_pesado">Vehicular Pesado</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo</label><select value={pavimento.tipo} onChange={e => setPavimento(d => ({ ...d, tipo: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="adoquinado">Adoquinado</option><option value="concreto">Concreto</option><option value="asfaltico">Asfaltico</option><option value="interlock">Interlock</option><option value="ceramico">Cerámico</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo Base</label><select value={pavimento.tipoBase} onChange={e => setPavimento(d => ({ ...d, tipoBase: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="c4">C4</option><option value="piedra_picada">Piedra Picada</option><option value="grava">Grava</option><option value="arena">Arena</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo Sello</label><select value={pavimento.tipoSello} onChange={e => setPavimento(d => ({ ...d, tipoSello: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="arena">Arena</option><option value="cemento">Cemento</option><option value="ninguno">Ninguno</option><option value="asfalto">Asfalto</option></select></div>
        ...
        {tab === 'redesInfraestructura' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo</label><select value={redInfraestructura.tipo} onChange={e => setRedInfraestructura(d => ({ ...d, tipo: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="agua_potable">Agua Potable</option><option value="alcantarillado_sanitario">Alcantarillado Sanitario</option><option value="alcantarillado_pluvial">Alcantarillado Pluvial</option></select></div>
              ...
              <div><label className="text-xs text-muted-foreground mb-1 block">Material</label><select value={redInfraestructura.material} onChange={e => setRedInfraestructura(d => ({ ...d, material: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="pvc">PVC</option><option value="cpvc">CPVC</option><option value="cobre">Cobre</option><option value="hdpe">HDPE</option><option value="concreto">Concreto</option><option value="fierro_fundido">Fierro Fundido</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Presión</label><select value={redInfraestructura.presion} onChange={e => setRedInfraestructura(d => ({ ...d, presion: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="baja">Baja</option><option value="media">Media</option><option value="alta">Alta</option></select></div>
        ...
        {tab === 'murosContencion' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              ...
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo de Muro</label><select value={muroContencion.tipo} onChange={e => setMuroContencion(d => ({ ...d, tipo: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="gravedad">Gravedad</option><option value="cantiliver">Cantilever</option><option value="atirantado">Atirantado</option><option value="tipo celular">Tipo Celular</option><option value="pantalla">Pantalla</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Cimentación</label><select value={muroContencion.tipoCimentacion} onChange={e => setMuroContencion(d => ({ ...d, tipoCimentacion: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="zapata_corrida">Zapata Corrida</option><option value="pilotes">Pilotes</option><option value="losa">Losa</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Tipo de Suelo</label><select value={muroContencion.tipoSuelo} onChange={e => setMuroContencion(d => ({ ...d, tipoSuelo: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="arcilla">Arcilla</option><option value="arena">Arena</option><option value="roca">Roca</option><option value="relleno_compactado">Relleno Compactado</option><option value="granular">Granular</option></select></div>
              <div><label className="text-xs text-muted-foreground mb-1 block">Drenaje</label><select value={muroContencion.tipoDrenaje} onChange={e => setMuroContencion(d => ({ ...d, tipoDrenaje: e.target.value }))} className="w-full text-xs px-3 py-2 rounded-lg border border-border outline-none focus:border-orange-400 bg-card"><option value="sin_drenaje">Sin Drenaje</option><option value="drenaje_superficial">Drenaje Superficial</option><option value="drenaje_interno">Drenaje Interno</option><option value="drenaje_completo">Drenaje Completo</option></select></div>
