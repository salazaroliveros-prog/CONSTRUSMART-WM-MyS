...
const ExportacionInteligente: React.FC = () => {
  ...
  const exportarJSON = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      toast.success(t('exportacion.json_exito', 'JSON exportado exitosamente'));
      setExportando(null);
    }, 300);
  }, [proyectos, movimientos, empleados, materiales, presupuestos, avances, t]);
  ...
  const generarPDF = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t('exportacion.pdf_exito', 'PDF exportado exitosamente'));
    } catch (err) {
      safeLogger.error(new Error('PDF error: ' + (err as Error).message));
      toast.error(t('exportacion.pdf_error', 'Error al generar PDF'));
    }
    setExportando(null);
  }, [proyectos, empleados, materiales, movimientos, t]);
  ...
  const eliminarReporte = (id: string) => {
    ...
    toast.success(t('exportacion.reporte_eliminado', 'Reporte eliminado'));
  };
  ...
  const exportarXLSX = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        toast.success(t('exportacion.xlsx_exito', 'Excel exportado exitosamente (6 hojas: EERR incluido)'));
      } catch (err) {
        safeLogger.error(new Error('XLSX error: ' + (err as Error).message));
        toast.error(t('exportacion.xlsx_error', 'Error al exportar Excel'));
      }
      setExportando(null);
    }, 500);
    ...
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto space-y-4">
      ...
      <div className="bg-card rounded-2xl shadow-sm border border-border p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setShowForm(true)} className="flex items-center gap-1 px-3 py-1.5 bg-primary text-primary-foreground rounded-lg text-xs font-medium hover:bg-primary/90">
            <Plus className="w-3.5 h-3.5" /> {t('exportacion.programar_reporte', 'Programar Reporte')}
          </button>
        </div>

        {showForm && (
          <div className="bg-purple-50 dark:bg-purple-950/20 rounded-xl p-4 mb-4 border border-purple-200 dark:border-purple-900/50 space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div>
                <input value={form.nombre} onChange={e => { setForm(prev => ({ ...prev, nombre: e.target.value })); if (formErrors.nombre) setFormErrors(f => ({ ...f, nombre: '' })); }} placeholder={t('exportacion.nombre_reporte', 'Nombre del reporte')} className={`w-full px-3 py-2 text-xs rounded-lg border border-purple-200 dark:border-purple-900/50 outline-none focus:border-ring ${formErrors.nombre ? 'border-red-500' : ''}`} />
              ...
              <select value={form.tipo} onChange={e => setForm(prev => ({ ...prev, tipo: e.target.value as ReporteProgramado['tipo'] }))} className={INPUT}>
                ...
                <option value="cubicacion">{t('exportacion.tipo_cubicacion', 'Cubicación')}</option>
                <option value="rendimientos">{t('exportacion.tipo_rendimientos', 'Rendimientos')}</option>
              </select>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <select value={form.formato} onChange={e => setForm(prev => ({ ...prev, formato: e.target.value as ExportFormat }))} className={INPUT}>
                ...
                <option value="pdf">{t('exportacion.formato_pdf', 'PDF (Ejecutivo)')}</option>
              </select>
              <select value={form.frecuencia} onChange={e => setForm(prev => ({ ...prev, frecuencia: e.target.value as ReporteProgramado['frecuencia'] }))} className={INPUT}>
                <option value="semanal">{t('exportacion.semanal', 'Semanal')}</option>
                <option value="mensual">{t('exportacion.mensual', 'Mensual')}</option>
              </select>
            </div>
            <input value={form.destinatarios} onChange={e => setForm(prev => ({ ...prev, destinatarios: e.target.value }))} placeholder={t('exportacion.destinatarios', 'Destinatarios (email separados por coma)')} className="w-full px-3 py-2 text-xs rounded-lg border border-purple-200 dark:border-purple-900/50 outline-none focus:border-ring" />
            <div className="flex gap-2">
              <button onClick={agregarReporte} className="flex-1 bg-primary text-primary-foreground hover:bg-primary/90 py-2 rounded-lg text-xs font-semibold">{t('exportacion.programar', 'Programar')}</button>
            ...
            {reportes.map(r => (
              <div key={r.id} className={`flex items-start justify-between p-3 rounded-lg border ${r.activo ? 'bg-card border-border' : 'bg-muted/30 border-border opacity-60'}`}>
                ...
                <div className="flex gap-1 shrink-0 ml-2">
                  <button onClick={() => ejecutarReporteAhora(r)} className="px-2 py-1 bg-primary text-primary-foreground rounded text-xs hover:bg-primary/90 flex items-center gap-1">
