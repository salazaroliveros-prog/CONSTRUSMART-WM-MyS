...
const Weather: React.FC = () => {
  ...
  }, [weather?.alertThreshold, alertThreshold]);
...
const WeatherHistoryChart: React.FC<{ history: WeatherHistoryItem[] }> = ({ history }) => {
  ...
  return (
    <div className="space-y-3">
      {!weather?.weatherData && !refreshing ? (
        <div className="text-center py-16 text-muted-foreground border border-dashed border-border rounded-2xl bg-card">
          <Cloud className="w-12 h-12 mx-auto mb-3 opacity-40" aria-hidden="true" />
          <p className="text-base font-medium">Sin datos climáticos</p>
          <p className="text-xs mt-1">Actualiza la información para calcular impacto en el proyecto</p>
        </div>
      ) : (
        <>
          {/* KPIs rápidos */}
