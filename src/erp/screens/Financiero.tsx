...
const Financiero: React.FC = () => {
  ...
  const centrosCosto = useMemo(() => proyectos.map(p => {
    ...
    const gas = movimientos.filter(m => m.proyectoId === p.id && m.tipo === 'gasto').reduce((a, b) => a + (b.costoTotal ?? b.monto ?? 0), 0);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto">
      <h1 className="text-xl sm:text-2xl font-black text-foreground flex items-center gap-2 mb-4"><Wallet className="w-6 h-6 text-violet-500" aria-hidden="true" /> Control Financiero y Caja</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <div className="lg:col-span-2 bg-card text-card-foreground rounded-2xl p-4 shadow-sm border border-border">
