-- ============================================================
-- SCRIPT COMPLETO: Auditoría y Alineación de Base de Datos
-- Ejecutar este script en Supabase SQL Editor
-- ============================================================

-- ============================================================
-- PARTE 1: AUDITORÍA DEL ESQUEMA ACTUAL
-- ============================================================
...
DECLARE
  ...
  is_obsolete BOOLEAN;
BEGIN
  RAISE NOTICE '=== AUDITORÍA DE ESQUEMA ACTUAL ===';
  ...
  -- Listar todas las tablas con estado
  FOR table_record IN 
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public' 
    ...
    ORDER BY table_name
  LOOP
    -- Verificar si es tabla obsoleta (basado en AGENTS.md)
    is_obsolete := FALSE;
    
    IF table_record.table_name IN (
      'erp_subcontratos',  -- Eliminado del estado (P3)
      'erp_rendimientos',  -- Eliminado de View, ALLOWED, screens (P4)
    ...
    IF is_obsolete THEN
      RAISE NOTICE '⚠️  OBSOLETA: %', table_record.table_name;
    ELSE
      RAISE NOTICE '✅ ACTIVA: %', table_record.table_name;
    END IF;
  END LOOP;
...
-- ============================================================
-- PARTE 2: LIMPIEZA DE TABLAS OBSOLETAS
-- ============================================================

DO $$
BEGIN
  RAISE NOTICE '=== LIMPIEZA DE TABLAS OBSOLETAS ===';
...
-- ============================================================
-- PARTE 3: CREACIÓN DE TABLAS GEOLÓGICAS (si no existen)
-- ============================================================

DO $$
BEGIN
  RAISE NOTICE '=== CREACIÓN DE TABLAS GEOLÓGICAS ===';
...
-- ============================================================
-- PARTE 4: CONFIGURACIÓN DE ÍNDICES
-- ============================================================
...
-- ============================================================
-- PARTE 6: CONFIGURACIÓN DE TRIGGERS
-- ============================================================
...
-- ============================================================
-- PARTE 7: CONFIGURACIÓN DE REALTIME
-- ============================================================
...
-- ============================================================
-- PARTE 8: INSERTAR SEED DATA (si está vacío)
-- ============================================================
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠️  Algunos datos pueden estar faltando';
  END IF;
