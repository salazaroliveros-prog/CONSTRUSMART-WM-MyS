-- Migration 054: Monitoring Functions for Scalability
-- Purpose: Create functions to monitor table growth and query performance
-- Risk: Low - functions are read-only and don't modify data
-- Rollback: Drops the monitoring functions
-- Expected Benefit: Better visibility into database growth and performance

-- ============================================================
-- Step 1: Function to get table sizes and row counts
-- ============================================================
...
-- ============================================================
-- Step 2: Function to get slow queries
-- ============================================================
...
-- ============================================================
-- Step 3: Function to get index usage statistics
-- ============================================================
...
-- ============================================================
-- Step 4: Function to get missing indexes (frequently queried columns without indexes)
-- ============================================================
...
CREATE OR REPLACE FUNCTION rollback_054_monitoring_functions()
RETURNS VOID AS $$
BEGIN
  -- Revoke permissions
  REVOKE EXECUTE ON FUNCTION get_table_sizes() FROM authenticated;
  REVOKE EXECUTE ON FUNCTION get_slow_queries() FROM authenticated;
  REVOKE EXECUTE ON FUNCTION get_index_usage() FROM authenticated;
  REVOKE EXECUTE ON FUNCTION get_missing_indexes() FROM authenticated;
  
  -- Drop functions
  DROP FUNCTION IF EXISTS get_table_sizes();
  DROP FUNCTION IF EXISTS get_slow_queries();
  DROP FUNCTION IF EXISTS get_index_usage();
  DROP FUNCTION IF EXISTS get_missing_indexes();
  
  RAISE NOTICE 'Migration 054 rolled back successfully';
END;
$$ LANGUAGE plpgsql;
