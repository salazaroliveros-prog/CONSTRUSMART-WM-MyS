...
const roleKey = env.ROLE_KEY_SECRET;
if (!url || !roleKey) {
  console.error('Faltan VITE_SUPABASE_URL o ROLE_KEY_SECRET en .env');
  process.exit(1);
}
...
async function run() {
  await admin.from('erp_proyectos').upsert({
    id: proyectoId,
  ...
  await admin.from('erp_presupuestos').upsert({
    id: presupuestoId,
  ...
  await admin.from('erp_movimientos').upsert([
  ...
  await admin.from('erp_empleados').upsert([
  ...
  await admin.from('erp_materiales').upsert([
  ...
  await admin.from('erp_avances').upsert([
  ...
  await admin.from('erp_seguimiento').upsert([
  ...
  await admin.from('erp_hitos').upsert([
  ...
  await admin.from('erp_riesgos').upsert([
  ...
  await admin.from('erp_proveedores').upsert([
  ...
  await admin.from('erp_ordenes_compra').upsert([
    { id: 'oc-01', proyecto_id: proyectoId, proveedor: 'Cementos Progreso', material: 'Cemento + Acero', cantidad: 150, monto: 42000000, estado: 'recibida', fecha: d(-55), items: [{ materialId: 'mat-01', cantidad: 500, precioUnitario: 78.5 }, { materialId: 'mat-02', cantidad: 120, precioUnitario: 142 }] },
  ]);

  await admin.from('erp_cuentas_cobrar').upsert([
    { id: 'cc-01', proyecto_id: proyectoId, cliente: 'Inmobiliaria Vista Hermosa S.A.', monto: 52000000, saldo: 25000000, fechaEmision: d(-60), fechaVencimiento: d(30), estado: 'parcial', factura: 'FAC-001-2026' },
  ]);

  await admin.from('erp_cuentas_pagar').upsert([
    { id: 'cp-01', proyecto_id: proyectoId, proveedor: 'Cementos Progreso', monto: 42000000, saldo: 12000000, fechaEmision: d(-55), fechaVencimiento: d(-20), estado: 'parcial', factura: 'FAC-501' },
  ]);

  await admin.from('erp_bitacora').upsert([
  ...
  await admin.from('erp_eventos_calendario').upsert([
  ...
  await admin.from('erp_publicaciones_muro').upsert([
  ...
  console.log('seed_completo:', { proyectoId, presupuestoId, modulo_movimientos: 6, modulo_empleados: 3, modulo_materiales: 3, modulo_avances: 5, modulo_seguimiento: 6, modulo_hitos: 3, modulo_riesgos: 2, modulo_proveedores: 2, modulo_ordenes: 1 });
}

run().catch(err => {
  console.error('seed_fallo:', err);
  process.exit(1);
});
